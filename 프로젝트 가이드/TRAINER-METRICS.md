...
### 산출식
```
점수 = (Σ 지표_달성률 × 배점) / (사용된 배점 합) × 100
```

### 지표별 달성률
| 지표 | 달성률 공식 | 비고 |
|---|---|---|
| 유효회원 | `min(active_members_avg / active_members_min, 1.0)` | 기준 초과 시 만점 cap |
| 월 세션 | `min(sessions_avg / sessions_min, 1.0)` | 기준 초과 시 만점 cap |
| 체험전환율 | `min(conversion_rate / conversion_min, 1.0)` | NULL 시 제외 |
| 재등록률 | `min(rereg_rate / rereg_min, 1.0)` | NULL 시 제외 |
| 소진일(8회) | `min(ref_days_per_8 / days_per_8_avg, 1.0)` | **낮을수록 좋음** (기준/실제). NULL 제외 |

### 배점 (기본값, SV 가 UI 에서 조정 가능)
| 지표 | 기본 배점 |
|---|---|
| 유효회원 | 20 |
| 월 세션 | 20 |
...
- `dongha_trainer_criteria.weight_*` 컬럼 5개. 합이 100 이어야 저장 가능 (밸리데이션).
- UI: 평가 기준값 아코디언 내 `[평가 기준]` / `[배점]` 섹션 구분.

### NULL 지표 처리 (정규화)
- 체험 없는 트레이너 → `conversion_rate = NULL` → 해당 배점(15점) 제외
- 사용된 배점 합 85점 중 획득 점수를 `× 100/85` 로 보정
- 이렇게 해야 "체험 없음" 트레이너가 불이익 없이 평가됨

### 테이블 `#` 컬럼
- 기본 정렬: 점수 내림차순 (1등부터)
- 정렬 바꿔도 # 는 정렬 기준 순위 숫자

---

## 6. 재계약 고려 판정

- **fail flags**: 5개 지표 (유효회원·세션·체험전환·재등록·소진일초과) 중 미달 카운트
...
4. **재등록 윈도우 제거**: 30일 → 45일 → **제한 없음** (ERP 원문 SQL: `p2.멤버십시작일 > 종료일` 만)
5. **재등록 분모**: `체험정규='정규'` → **`"전환재등록" IN ('재등록','휴면','미등록') OR (NULL AND 체험정규 NULL)`** (ERP 원문)
6. **재등록 분자**: 하이브리드 — `"전환재등록" = '재등록'` **또는** `(NULL AND 체험정규 NULL AND EXISTS 후속 정규 PT)` (ERP 원문)
7. **체험전환 분모**: `체험정규='체험'` → **`"전환재등록" IN ('체험전환','미전환')`** (ERP 원문)
8. **비정규 상품 제외**: 쿠폰·이벤트·체험팩·무료·증정·선물·복지 키워드 포함 멤버십명 + 무제한(총횟수≥99999) 전 쿼리 제외. 상품명 리스트는 `/fde-api/dongha/trainers/debug/non-regular-products` 로 확보 후 `_COUPON_PATTERN` 및 문서 반영

## 11. 남은 차이 (의도적)

| 항목 | ERP | FDE | 이유 |
|---|---|---|---|
| 유효회원 체험 포함 | 항상 포함 | **토글** (기본 OFF = 정규만) | 트레이너 평가 기본값은 정규 PT 가 적합. 필요 시 토글로 ERP 숫자 일치 |
| 세션 트레이너 매칭 | 이름 문자열 | user_btrainer JOIN | 동명이인 방어. PR D 에서 이름 기반으로 전환 검토 중 |
| 무제한권 | 포함 | **제외** | FDE 는 임직원/특수계약 제외한 유상 PT 트레이너 평가 목적 |
| 음수 gap (오버랩) 재등록 | 인정 | 미포함 (종료일 이후만) | ERP 원문 SQL (`p2.시작일 > 종료일`) 이 실제로 음수 gap 미포함 — FDE 도 동일 |

## 12. ERP 원문 SQL (부록, 2026-04-23 기준)

ERP `/pt/trainer` 백엔드 (`backend/routers/pt.py:1677-1946`) 에서 실행되는 SQL 원문. FDE 쿼리 수정 시 참조용. 모두 `raw_data_pt` / `raw_data_reservation` 만 사용.

### ERP #1 — 월별 유효회원
```sql
SELECT "담당트레이너" AS trainer, '{month}' AS month,
       COUNT(DISTINCT user_id) AS count
FROM raw_data_pt
WHERE ("환불여부" IS NULL OR "환불여부" != '환불')
  AND "멤버십시작일" <= '{month_end}'
  AND "멤버십종료일" >= '{month_start}'
  [AND "지점명" = '{branch}']
  [AND "담당트레이너" = '{trainer}']
GROUP BY "담당트레이너"
...
### ERP #2 — 월별 세션
```sql
SELECT "트레이너" AS trainer, '{month}' AS month, COUNT(*) AS 수업수
FROM raw_data_reservation
WHERE "프로그램명" = 'PT'
  AND "수업날짜" >= '{month_start}' AND "수업날짜" <= '{month_end}'
  AND "예약취소" != '취소'
  [AND "지점명" = '{branch}']
GROUP BY "트레이너";
```
...
SELECT "담당트레이너" AS trainer, '{month}' AS month,
       COUNT(*) AS 체험수,
       COUNT(*) FILTER (WHERE "전환재등록" = '체험전환') AS 체험전환수
FROM raw_data_pt
WHERE "전환재등록" IN ('체험전환', '미전환')
  AND "멤버십종료일" >= '{month_start}' AND "멤버십종료일" <= '{month_end}'
  [AND "지점명" = '{branch}']
  [AND "담당트레이너" = '{trainer}']
GROUP BY "담당트레이너"
HAVING COUNT(*) > 0;
...
레퍼런스: `pt_트레이너_지표_산출로직.md` (2026-04-23, 버핏볼트 ERP 팀 공유).
