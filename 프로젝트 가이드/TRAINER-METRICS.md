...
### ④ 재등록률 (기간 누적)
- **출처**: `raw_data_pt`
- **정의**: 기간 중 정규 PT 멤버십이 만료된 회원 중 **30일 내 재등록**한 비율
- **수식 (per-ending 30일 윈도우)**:
  ```sql
  ending = 정규 만료자 (월별: 멤버십종료일 ∈ 월, 무제한 제외)
  분모 = COUNT(DISTINCT ending.회원연락처)
  분자 = COUNT(DISTINCT ending.회원연락처) WHERE EXISTS (
    SELECT 1 FROM raw_data_pt pt2
    WHERE pt2.체험정규='정규'
      AND pt2.전환재등록='재등록'
      AND pt2.회원연락처 = ending.contact
      AND pt2.멤버십시작일 BETWEEN ending.end_date AND ending.end_date + INTERVAL '30 days'
  )
  ```
- ⚠️ **Per-ending 윈도우 사용 이유**: 기간 전체 윈도우(월초~말+30일 같은 넓은 범위)는 다른 멤버십의 재등록까지 잡혀 오탐. 각 종료의 end_date 기준 30일 내만 검사해야 정확.
...
| **환불 제외** | 현재 미적용 (TODO). `raw_data_pt` 에 결제상태 컬럼 없음. `raw_data_mbs` JOIN 으로 추후 구현 필요 |
...
| 이슈 | 우선순위 | 메모 |
|---|---|---|
| 환불 멤버십 제외 미적용 | 중 | `raw_data_mbs` JOIN 으로 구현. 현재 환불 건 약간 노이즈로 포함 |
