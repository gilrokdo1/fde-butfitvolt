...
### ④ 재등록률 (기간 누적)
- **출처**: `raw_data_pt`
- **정의**: 기간 중 정규 PT 멤버십이 만료된 회원 중 **45일 내 재등록**한 비율 (ERP /pt/trainer 동일 윈도우)
- **수식 (per-ending 45일 윈도우)**:
  ```sql
  ...
  분자 = COUNT(DISTINCT ending.회원연락처) WHERE EXISTS (
    ...
    WHERE pt2.체험정규='정규'
      ...
      AND pt2.멤버십시작일 BETWEEN ending.end_date AND ending.end_date + INTERVAL '45 days'
  )
  ```
