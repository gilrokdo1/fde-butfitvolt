...
- 배포는 GitHub Actions 자동 배포 (PEM은 GitHub Secret으로만 보관, 팀원에게 미공유)
- 멤버들은 백엔드 코드, DB 테이블, API 라우터 모두 자유롭게 추가 가능 (PR 머지 시 자동 반영)
...
- **배포** — PR 머지 시 GitHub Actions가 자동 배포 (팀원은 EC2 접속 불필요)
